...
class VSHorsesDataGenerator: DataGeneratorEntrypoint {
  ...
  private inner class TrimmedHorseArmorItemModelProvider(output: FabricDataOutput):
    ...
    private fun getModIdentifierWithTrim(
      ...
      itemModelGenerator.suffixTrim(itemId, trimMaterialName).path
    )
    ...
    private fun getModIdentifierWithTrim(
      ...
      itemModelGenerator, itemId, trimMaterial.getAppliedName(armorMaterial)
    )
    ...
    private fun generateHorseArmorItemModel(
      ...
      val (model, textureMap) = if (textureOverlayId == null) {
        Pair(Models.GENERATED, TextureMap.layer0(textureId))
      } else {
        Pair(
          Models.GENERATED_TWO_LAYERS,
          TextureMap.layered(textureId, textureOverlayId)
        )
      }
      model.upload(
        modelId, textureMap, itemModelGenerator.writer
      ) { id, textures ->
        ...
        for (trimMaterial in ItemModelGenerator.TRIM_MATERIALS) {
          ...
          predicateJsonObj.addProperty(
            ItemModelGenerator.TRIM_TYPE.path, trimMaterial.itemModelIndex
          )
          val overrideJsonObj = JsonObject()
          overrideJsonObj.addProperty(
            "model", getModIdentifierWithTrim(
              itemModelGenerator, id, trimMaterial, armorMaterial
            ).toString()
          )
    ...
    private fun generateHorseArmorTrimItemModels(
      ...
      for (trimMaterial in ItemModelGenerator.TRIM_MATERIALS) {
        ...
        val trimTextureId = getModIdentifierWithTrim(
          itemModelGenerator, modelId, trimMaterialName
        )
        ...
        val (model, textureMap) = if (textureOverlayId == null) {
          ...
          Pair(
            Models.GENERATED_THREE_LAYERS, TextureMap.layered(
              textureId, textureOverlayId, trimTextureOverlayId
            )
          )
        }
    ...
    override fun generateItemModels(itemModelGenerator: ItemModelGenerator) {
      Registries.ITEM.filterIsInstance<AnimalArmorItem>().filter {
        it.type == AnimalArmorItem.Type.EQUESTRIAN
      }.forEach {
        ...
        generateHorseArmorItemModel(
          itemModelGenerator, modelId, textureId, textureOverlayId, it.material
        )
